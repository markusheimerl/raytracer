...
__device__ Ray get_camera_ray_gpu(const Camera* camera, float x, float y, float aspect) {
    Vec3 forward = vec3_normalize_gpu(vec3_sub_gpu(camera->look_at, camera->position));
    Vec3 right = vec3_normalize_gpu(vec3_cross_gpu(forward, camera->up));
    Vec3 camera_up = vec3_cross_gpu(right, forward);
    
    float scale = tanf((camera->fov * 0.5f) * M_PI / 180.0f);
    float ray_x = (2.0f * x - 1.0f) * aspect * scale;
    float ray_y = (1.0f - 2.0f * y) * scale;

    Vec3 ray_dir = vec3_normalize_gpu(vec3_add_gpu(
        vec3_add_gpu(
            vec3_mul_gpu(right, ray_x),
            vec3_mul_gpu(camera_up, ray_y)
        ),
        forward
    ));
