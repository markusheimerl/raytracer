...
void convert_mesh_to_gpu(const Mesh* cpu_mesh, GPUMesh* gpu_mesh) {
    ...
    CHECK_CUDA(cudaMalloc(&gpu_mesh->d_triangles, cpu_mesh->triangle_count * sizeof(Triangle)));
    CHECK_CUDA(cudaMemcpy(gpu_mesh->d_triangles, cpu_mesh->triangles, 
                         cpu_mesh->triangle_count * sizeof(Triangle), cudaMemcpyHostToDevice));
    
    // Copy texture data to GPU
    size_t texture_size = cpu_mesh->texture_width * cpu_mesh->texture_height * 4;
    CHECK_CUDA(cudaMalloc(&gpu_mesh->d_texture_data, texture_size));
    CHECK_CUDA(cudaMemcpy(gpu_mesh->d_texture_data, cpu_mesh->texture_data, 
                         texture_size, cudaMemcpyHostToDevice));
    
    // Copy BVH to GPU
    gpu_mesh->d_bvh_root = copy_bvh_to_gpu(cpu_mesh->bvh.root);
    
    // Copy other properties
    gpu_mesh->triangle_count = cpu_mesh->triangle_count;
    gpu_mesh->texture_width = cpu_mesh->texture_width;
    gpu_mesh->texture_height = cpu_mesh->texture_height;
    gpu_mesh->transform = cpu_mesh->transform;
}
...
__device__ Vec3 sample_mesh_texture_gpu(const GPUMesh* mesh, float u, float v) {
    ...
    int idx = (y * mesh->texture_width + x) * 4;
    return (Vec3){
        mesh->d_texture_data[idx] / 255.0f,
        mesh->d_texture_data[idx + 1] / 255.0f,
        mesh->d_texture_data[idx + 2] / 255.0f
    };
}